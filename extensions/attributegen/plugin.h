...
public:
  APINode(){};
  APINode(const std::string& value) : val_(value){};

  std::string val_;
  absl::flat_hash_map<std::string, std::shared_ptr<APINode>> children_;
  APILeaf leaf_;
};
...
class APIMatch {
public:
  void addAPI(const std::string& condition, const std::string& value);

  bool match(const std::string& method, const absl::string_view url_path, std::string* val,
             absl::string_view* match_path) const;
  // static api path
  //                 method                            url_path     value
  absl::flat_hash_map<std::string, absl::flat_hash_map<std::string, std::string>> static_;
  // param api path
  //                  method       node
  absl::flat_hash_map<std::string, std::shared_ptr<APINode>> param_;
};
...
public:
  explicit AttributeGenerator(APIMatch& api_match, EvalPhase phase,
                              const std::string& output_attribute,
                              const std::vector<Match>& matches)
      : api_match_(api_match), phase_(phase), output_attribute_(output_attribute),
